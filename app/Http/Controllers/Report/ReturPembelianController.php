...
class ReturPembelianController extends Controller
{
    ...
    private $_limit  = 25;
    private $_search = [];
    private $_route  = 'report-retur-pembelian';
    ...
    public function index()
    {
        ...
        if (request('id')) {
            $pembelian = ReturPembelian::where('id',(int)request('id'))->first();
        }
        $search = $this->_search;

        $param   = [];

        $pembelian = new ReturPembelian;
        ...
        $pembelian = $pembelian->with('transaksi');
        if (get('export', false)) {
            if($this->_page == 1)
            {
        ...
        foreach ($pembelian as $key => $value) {
            $data[] = [
                'number'        => ++$i,
                'no'            => $value['transaksi']['pembelian']['no_transaksi'],
                'tanggal'       => date('d M Y H:i',strtotime($value['transaksi']['pembelian']['tanggal'])),
        ...
        $data          = [
            ...
            'breadcrumb'         => [
                ...
                ['url' => '#', 'text' => '<i class="fa fa-tag"></i> Report ReturPembelian'],
            ],
            'header_title'       => 'Report',
            'header_description' => 'ReturPembelian',
            'table'              => $table,
            'route'              => $this->_route,
            'total'              => $count,
        ...
        return view("report/retur_pembelian", $data);
    ...
    function export_content($data,$get)
    {
        ...
        $report_title = 'Report Data Retur Pembelian '. $_title;
        ...
        {     
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue($abj.$i, $value['transaksi']['pembelian']['no_transaksi']);
            $objPHPExcel->getActiveSheet()->getStyle($abj.$i)->applyFromArray($style);
            $abj++;
            $objPHPExcel->getActiveSheet()->SetCellValue($abj.$i, date('d M Y H:i',strtotime($value['transaksi']['pembelian']['tanggal'])));
            $objPHPExcel->getActiveSheet()->getStyle($abj.$i)->applyFromArray($style);
            $abj++;
